...
with st.form("Country selection"):
    if (region1 == 'Middle East/Northern Africa'):
        country1 = st.selectbox(
            'Which country in the {} are you interested in?'.format(region1), mena_countries)
    else:
        country1 = st.selectbox(
            'Which country in the {} are you interested in?'.format(region1), sea_countries)
...
if submitted:
    ...
    m1 = procCountry(bodies1)
    st.write(f'country: {country1} Mean : {m1}')
    completed = True
...
if completed:
    with st.form("do another country?"):
        do_2nd_country = st.form_submit_button("Would you like to compare to another country?")
            ...
            with st.form("Country selection"):
                ...
                if (region2 == 'Middle East/Northern Africa'):
                    country2 = st.selectbox(
                        'Which country in the {} are you interested in?'.format(region2), mena_countries)
                else:
                    country2 = st.selectbox(
                        'Which country in the {} are you interested in?'.format(region2), sea_countries)
                submitted2 = st.form_submit_button("Click to scrape for articles for {}.".format(country2))
            
            if submitted2:
                bodies2 = getArticle(country2)
                m2 = procCountry(bodies2)
